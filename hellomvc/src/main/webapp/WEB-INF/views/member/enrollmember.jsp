...
<section id=enroll-container>
       ...
       <table>
		...
		<tr>
			<td>
				<input type="text" id="hobby" list="hobbylist">
				<datalist id="hobbylist">
				</datalist>
				<script>
					$("#hobby").keyup(e=>{
						$.get("<%=request.getContextPath()%>/ajax/hobbies.do?keyword="+e.target.value)
						.done(data=>{
							console.log(data);
							$("#hobbylist").html("");
							data.forEach(e=>{
								const opt=$("<option>").attr("value",e).text(e);
			...
			((request)=>{
				return e=>{
					...
					request=setTimeout(()=>{
						...
						fetch("<%=request.getContextPath()%>/ajax/idduplicate.do?id="+userId)
						.then(response=>{
